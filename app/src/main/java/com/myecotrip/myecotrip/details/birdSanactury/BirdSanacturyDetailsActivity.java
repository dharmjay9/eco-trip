...
import com.myecotrip.myecotrip.details.ecoTrail.EcoTrailDetailsAdapter;
...
public class BirdSanacturyDetailsActivity extends BaseActivity {

    public static final String BIRD_ID="ecotrail_id";
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String id = getIntent().getStringExtra(BIRD_ID);
        CommonDetailsRequest commonDetailsRequest = new CommonDetailsRequest();
        commonDetailsRequest.setPark_id(id);
        restClient.getBirdSanctury(commonDetailsRequest, new MyEcoTripCallBack<BirdSanacturyResponse>() {
            @Override
            ...
            @Override
            public void onSuccess(BirdSanacturyResponse ecoDetailsResponse) {

                BirdSanacturyResponse.ContentBean.BirdSanctuaryBean ecoTrailRowdata = ecoDetailsResponse.getContent().getBirdSanctuary().get(0);
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    tvDetails.setText(Html.fromHtml(ecoTrailRowdata.getPark_name(), Html.FROM_HTML_MODE_COMPACT));
                } else {
                    tvDetails.setText(Html.fromHtml(ecoTrailRowdata.getPark_description()));
                }
                tvTitle.setText(ecoTrailRowdata.getPark_name());
                SanacturyActivityAdapter ecoTrailDetailsAdapter = new SanacturyActivityAdapter(ecoDetailsResponse.getContent().getActivities(), BirdSanacturyDetailsActivity.this);
               ...
               // mainLayout.setVisibility(View.VISIBLE);
                Picasso.with(BirdSanacturyDetailsActivity.this).load(CommonUtils.getImageUrl((ecoTrailRowdata.getLogo()))).into(mIvDetails);
            }
        });
    ...
    protected void initView() {
        setContentView(R.layout.activity_bird_sanatury);
